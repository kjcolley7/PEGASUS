...
//  PegasusEar
//
//  Created by Kevin Colley on 11/19/2022.
//
...
#define PEG_PATCH_INPUT_FILE "SiliconBridge.peg"


typedef struct PegPatchPlugin {
	PegPlugin base;
...
static bool cb_portWrite(void* portWrite_cookie, uint8_t port_number, EAR_Byte byte) {
	PegPatchPlugin* ctx = portWrite_cookie;
	if(port_number == 0) {
...
static bool cb_portRead(void* portRead_cookie, uint8_t port_number, EAR_Byte* out_byte) {
	PegPatchPlugin* ctx = portRead_cookie;
...
static void cb_destroy(PegPlugin* plugin) {
	PegPatchPlugin* ctx = (PegPatchPlugin*)plugin;
	free(ctx->flag);
	free(ctx);
}
...
PegPlugin* PegPlugin_Init(EAR* ear, PegasusLoader* pegload) {
	...
	PegPatchPlugin* ctx = calloc(1, sizeof(*ctx));
	if(!ctx) {
		return NULL;
	}
	...
	char flag_data[100];
	FILE* flag_fp = fopen("/ctf/flag.txt", "r");
	if(!flag_fp) {
...
int main(int argc, char** argv, char** envp) {
	...
	// as a plugin.
	char* runpeg_argv[] = {
		"runpeg", "--plugin", argv[0], PEG_PATCH_INPUT_FILE, NULL
	};
