...
* One character is for down, and one character is for right. Each node uses
  different characters to denote down and right, to prevent a simple brute
  force attack from being effective.
...
_choice_charset = string.digits + string.ascii_uppercase + string.ascii_lowercase

class AsmGrid:
	def __init__(self, width, height, max_value, seed=None):
		self.width = width
		self.height = height
	...
	def _asm_block(self, x, y, value):
		# Pick a random character as the character to move right
		right_char = self.prng.choice(_choice_charset)
		
		# Pick a random character as the character to move down,
		# making sure not to select the same one as the right char
		remaining = "".join(c for c in _choice_charset if c != right_char)
		down_char = self.prng.choice(remaining)
...
def asm_grid(width, height, max_value, seed=None):
	builder = AsmGrid(width, height, max_value, seed)
